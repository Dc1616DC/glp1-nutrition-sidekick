...
function shouldSendReminder(
  ...
  if (reminderTimeInMinutes < 0) return false;
...
async function sendPushNotification(
  ...
  try {
    ...
    await Promise.all(
      fcmTokens.map(async (token) => {
        try {
          ...
          console.error(
            `Failed sending to token for user ${userId}: ${token}`,
            err?.message || err
          );
        }
      })
    ...
    console.log(
      `Successfully sent ${successCount}/${fcmTokens.length} notifications for user ${userId}`
    );
 ...
 */
function getMealName(mealKey: string): string {
 ...
 */
export const checkAndSendMealReminders = functions
  .region('us-central1')
  .pubsub
    ...
    try {
      ...
      const reminderPromises = usersSnapshot.docs.map(async (userDoc) => {
        ...
        for (const mealType of mealTypes) {
          const timeString = notificationSettings[mealType as keyof NotificationSettings];
          ...
          if (shouldSendReminder(reminderTimeInMinutes, currentTimeInMinutes)) {
            ...
            await sendPushNotification(
              ...
              fcmTokens,
              `Time for ${mealName}!`,
              `It's time for your scheduled ${mealName.toLowerCase()}. Don't forget to eat mindfully.`,
              {
                mealType,
 ...
 */
export const triggerMealReminders = functionsV2.https.onRequest(
  {
    region: 'us-central1',
  },
  async (req, res) => {
    ...
    try {
      ...
      const reminderPromises = usersSnapshot.docs.map(async (userDoc) => {
        ...
        if (!fcmTokens?.length || !notificationSettings) {
        ...
        for (const mealType of mealTypes) {
          const timeString =
            notificationSettings[mealType as keyof NotificationSettings];
          if (!timeString) continue;
          ...
          if (diff <= 15 || diff >= 1425) {
            await sendPushNotification(
              uid,
              fcmTokens,
              `Time for ${getMealName(mealType)}!`,
              `It's time for your scheduled ${getMealName(
                mealType
              ).toLowerCase()}. Don't forget to eat mindfully.`,
              {
                mealType,
                scheduledTime: timeString,
                notificationType: 'meal-reminder',
 ...
 */
export const sendTestNotification = functionsV2.https.onCall(
  {
    region: 'us-central1',
  },
  async (request) => {
  ...
  try {
    ...
    if (fcmTokens.length === 0) {
      throw new functions.https.HttpsError(
        'failed-precondition',
        'No FCM tokens found for this user. Please enable notifications first.'
      );
    }
    ...
    await sendPushNotification(
      ...
      fcmTokens,
      'Test Notification',
      'This is a test notification from GLP-1 Nutrition Companion',
      {

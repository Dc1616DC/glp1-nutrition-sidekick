...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      // Step 2: Get accurate nutrition data from Spoonacular
      console.log('📊 Step 2: Fetching accurate nutrition data from Spoonacular...');
      for (let i = 0; i < generatedRecipes.length; i++) {
        const recipe = generatedRecipes[i];
        
        try {
          // Get nutrition for all ingredients using batched Spoonacular lookup
          const nutritionData = await spoonacularNutritionService.getNutritionForIngredients(recipe.ingredients);
          
          // Calculate totals
          const totals = spoonacularNutritionService.calculateTotalNutrition(nutritionData);
          
          // Update recipe with accurate nutrition
          recipe.nutritionTotals = totals;
          
          // Update individual ingredient nutrition
          recipe.ingredients.forEach((ingredient, index) => {
            if (nutritionData[index]) {
              ingredient.nutrition = nutritionData[index];
            }
          });

          // Log nutrition source for transparency
          const sources = nutritionData.map(n => n.source);
          const uniqueSources = [...new Set(sources)];
          console.log(`✅ Recipe "${recipe.title}" nutrition from: ${uniqueSources.join(', ')}`);

        } catch (nutritionError) {
          console.error(`❌ Nutrition calculation failed for recipe ${i}:`, nutritionError);
          
          // If Spoonacular fails completely, keep Grok's estimated nutrition as fallback
          console.log(`🔄 Using Grok's nutrition estimates for "${recipe.title}"`);
        }
      }
      ...
      return NextResponse.json({ 
        ...
        source: 'grok+spoonacular',
        generationTime: duration,
        disclaimer: 'Recipes generated by AI and validated for GLP-1 compatibility. Nutrition data from Spoonacular API with caching and fallback systems.'
      });
